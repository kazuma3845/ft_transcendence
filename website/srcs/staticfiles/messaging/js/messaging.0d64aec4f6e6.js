...
function startConversation(selectedUserId) {
    // Envoyer une requête POST au backend pour créer une nouvelle conversation
    fetch('/api/messaging/conversations/create_conversation', {
        ...
        body: JSON.stringify({
            participant_id: selectedUserId  // Inclure l'ID de l'utilisateur sélectionné dans la requête
        })
    })
    .then(response => {
        if (!response.ok) {
    ...
    .then(data => {
        ...
        // Mettre à jour l'interface pour afficher la nouvelle conversation
        loadConversations();  // Recharger la liste des conversations après la création
        closeSearchUserModal();  // Fermer la modal de recherche d'utilisateur
    })
    .catch(error => {
