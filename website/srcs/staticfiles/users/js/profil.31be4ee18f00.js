...
async function computeStats(results) {
  ...
  // Calculer la winstreak actuelle
  let currentWinStreak = 0;
  for (let i = results.length - 1; i >= 0; i--) {
    if (results[i].winner === userInfo.username) {
      currentWinStreak++;
    } else {
      break; // Arrêter de compter dès la première défaite rencontrée
    }
  }
  ...
  results.forEach((result) => {
    if (result.winner !== userInfo.username) {
