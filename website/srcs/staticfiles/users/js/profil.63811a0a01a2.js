...
async function fetchUserStats(username) {
  return fetch(`/api/users/profiles/game-sessions/?username=${username}`)
    .then((response) => {
      if (!response.ok) {
        // Si on reçoit un 404, on retourne le message et on arrête là
        if (response.status === 404) {
          return response.json().then((data) => {
            return { message: data.message, username };
          });
        }
    ...
    .then((games) => {
      if (games.message) {
        // Si aucune partie n'a été trouvée, on retourne le message et on arrête ici
        return games;
      }
    ...
    .then((results) => {
      // Si aucun message d'erreur, on calcule les statistiques
      if (results.message) {
        return results; // Si c'était un message d'erreur, on l'arrête ici
      }
