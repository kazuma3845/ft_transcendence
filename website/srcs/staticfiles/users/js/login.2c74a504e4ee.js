      ...
      fetch("/api/users/profiles/login/", {
        ...
        .then(async (data) => {
          if (data.error) {
            ...
            await fetchUserInfo();
            loadModal("Logged In Successfully", `Welcome back ${userInfo.user.username}, you've been missed 💜`);
            updateHeader();
  ...
  fetch("api/users/logout/", {
    ...
    .then((response) => {
      if (response.ok) {
        ...
        loadModal("Logged Out Successfully", `Goodbye ${userInfo.user.username}, you will be missed...💔`);
        userInfo = null;
        updateHeader();
