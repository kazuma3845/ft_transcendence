...
async function loadFriends() {
  try {
    ...
    console.log("Nombre de demandes d'amis:", friendRequests.length);

    const friendRequestsBlock = document.getElementById("friend-requests-block");
    friendRequestsBlock.innerHTML = ""; // Vider le conteneur des demandes d'amis
    ...
    friendRequests.forEach(async (request) => {
      ...
      // Créer un conteneur pour l'avatar et les boutons avec positionnement relatif
      const requestContainer = document.createElement("div");
      requestContainer.className = "friend-request position-relative d-inline-block"; // Position relative pour positionner les badges sur l'avatar
      ...
      // Ajouter le lien (avec l'avatar) au conteneur de la demande d'ami
      requestContainer.appendChild(requestLink);

      // Créer le badge d'acceptation (avec un "v") en haut à droite
      const acceptBadge = document.createElement("span");
      acceptBadge.className = "badge rounded-pill bg-success text-white position-absolute top-0 end-0 m-1";
      acceptBadge.innerHTML = '<i class="fas fa-check"></i>'; // Icône "v"
      acceptBadge.style.cursor = "pointer";
      acceptBadge.addEventListener("click", () => {
        acceptFriendRequest(request.from_user);
      });

      // Créer le badge de refus (avec un "x") en haut à gauche
      const rejectBadge = document.createElement("span");
      rejectBadge.className = "badge rounded-pill bg-danger text-white position-absolute top-0 start-0 m-1";
      rejectBadge.innerHTML = '<i class="fas fa-times"></i>'; // Icône "x"
      rejectBadge.style.cursor = "pointer";
      rejectBadge.addEventListener("click", () => {
        rejectFriendRequest(request.from_user);
      });

      // Ajouter les badges au conteneur de la demande d'ami
      requestContainer.appendChild(rejectBadge); // En haut à gauche
      requestContainer.appendChild(acceptBadge); // En haut à droite

      // Ajouter le conteneur complet au bloc de demandes d'amis
      friendRequestsBlock.appendChild(requestContainer);
    });
  } catch (error) {
    console.error("Erreur lors de la récupération des amis ou des demandes d'amis:", error);
