...
document.addEventListener("DOMContentLoaded", function() {
  ...
  // Clic sur la bannière pour ouvrir le sélecteur de fichier
  ...
  // Gérer le changement de fichier pour la bannière
  bannerUpload.addEventListener("change", function(event) {
    const file = event.target.files[0];
    if (file) {
      const reader = new FileReader();
      reader.onload = function(e) {
        bannerImg.src = e.target.result; // Met à jour l'image avec le fichier sélectionné
        // Envoyer le fichier au serveur et rafraîchir la page
  ...
  avatarUpload.addEventListener("change", function(event) {
    ...
    if (file) {
      const reader = new FileReader();
      reader.onload = function(e) {
        avatarImg.src = e.target.result; // Met à jour l'image avec le fichier sélectionné
        // Envoyer le fichier au serveur et rafraîchir la page
  ...
  function uploadImage(file, type) {
    ...
    // Remplacez l'URL avec celle de votre serveur
    fetch("/api/upload-image", {
      ...
      .then(data => {
        if (data.success) {
          ...
          // Rafraîchit la page pour recharger la nouvelle image depuis le serveur
          window.location.reload(); // Actualise la page pour afficher la nouvelle image
        } else {
...
function displayNoGame() {
  ...
  const message = "Aucune partie jouée par l'utilisateur.";
  let index = 0;
