...
async function loadFriends(user) {
  try {
    ...
    const friendRequests = user.friends_requests;
    ...
    friends.forEach((friend) => {
      ...
      friendLink.href = `/profile/?username=${friend.user.username}`;
    ...
    friendRequests.forEach(async (request) => {
      console.log("TRIGGGEEEEEER");
      const friendRequestProfile = await fetchUserInfoLight(request.from_user);
      const requestLink = document.createElement("a");
      requestLink.href = `/profile/?username=${friendRequestProfile.user.username}`; // Même logique pour les demandes d'amis

      const requestImg = document.createElement("img");
      requestImg.src = friendRequestProfile.avatar ? friendRequestProfile.avatar : "/static/users/avatars/avatar.png"; 
