...
function animateTitle() {
  ...
  function typeWriter() {
    ...
    const deleteUntil = titleSteps[stepIndex].deleteUntil;

    if (!isDeleting) {
      // Ajouter un caractère à la fois
      currentText = fullText.substring(0, currentText.length + 1);
    } else {
      // Supprimer un caractère à la fois jusqu'à la position deleteUntil
      currentText = currentText.substring(0, currentText.length - 1);
    }

    // Mise à jour du texte dans l'élément HTML
    ...
    if (!isDeleting && currentText === fullText) {
      ...
      // Si la suppression a atteint le point défini (deleteUntil), on passe à l'étape suivante
      isDeleting = false;
      stepIndex++;
    ...
    // Relancer la fonction à la vitesse définie
