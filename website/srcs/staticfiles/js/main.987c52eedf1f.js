...
async function animateTitle() {
  ...
  const titleSteps = [
    { text: "Transend", delete: 5 }, // Écrit "Transend" puis supprime jusqu'à "Trans"
    { text: "Transcenden", delete: 8 }, // Écrit "Transcenden" puis supprime jusqu'à "Transcend"
    { text: "Transcendance", delete: 12 }, // Écrit "Transcendance" en entier
  ];

  let speed = 100;
  let pauseBetweenSteps = 500; // Pause entre chaque étape

  for (let x = 0; x < titleSteps.length; x++) {
    const fullText = titleSteps[x].text;
    const deleteUntil = titleSteps[x].delete;

    // Écriture du texte
    for (let y = 0; y <= fullText.length; y++) {
      const currentText = fullText.substring(0, y);
      titleElement.textContent = currentText;
      await sleep(speed); // Pause entre chaque caractère
    }

    // Ajoute une petite pause après avoir écrit complètement le texte
    await sleep(pauseBetweenSteps);

    // Suppression du texte après l'écriture
    for (let z = fullText.length; z > deleteUntil; z--) {
      const currentText = fullText.substring(0, z);
      titleElement.textContent = currentText;
