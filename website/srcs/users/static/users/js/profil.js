...
function createWinStreakBlock() {
  const winStreak = userInfo.stats.winStreak; // Récupérer la win streak de l'utilisateur
  const winStreakTitle = document.querySelector("#win-streack-block h2");
  winStreakTitle.textContent = `Win Streak: ${winStreak}`; // Mettre à jour le titre

  // Récupérer l'iframe et le conteneur
  const flameIframe = document.getElementById("flame-iframe");

  // Choisir l'URL du GIF Giphy en fonction de la win streak
  let flameGif;
  if (winStreak === 0) {
...
// Fonction pour créer le graphique Winrate
