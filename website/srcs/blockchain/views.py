...
def set_game_session_scores(game_session_id, players, scores):
    try:
        ...
        print(f"Type of players: {type(players)} and type of first element if available: {type(players[0]) if players else 'Empty list'}")
        print(f"Type of scores: {type(scores)} and type of first element if available: {type(scores[0]) if scores else 'Empty list'}")
        print(f"Current chain ID: {chain_id}")

        transaction = contract.functions.setGameSessionScores(
            game_session_id, players, scores
        ).build_transaction(
            {
                "from": account,
...
def get_scores_by_game_session(game_session_id):
    try:
        usernames, scores = contract.functions.getScoresByGameSession(
            game_session_id
        ).call()
...
def retrieve_game_session_scores(request):
    if request.method == "GET":
        ...
        print("L'ID de session est :", game_session_id, "et est de type", type(game_session_id))
        ...
        return JsonResponse({"game_session_id": game_session_id, "scores": player_scores})
    return JsonResponse({"error": "Invalid request method"}, status=405)
